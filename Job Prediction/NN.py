...
onehot = pickle.load(open('onehot.p', 'rb'))
lexicon = pickle.load(open('lexicon.p', 'rb'))
jobs = pickle.load(open('jobs.p', 'rb'))

def divide_sets(train_set, test_size = 0.2):
    random.shuffle(onehot)
    xx = [a[0] for a in onehot]
    yy = [a[1] for a in onehot]
    
    testing_size = int(test_size*len(onehot))
...
train_x,train_y,test_x,test_y = divide_sets(onehot)
...
def train_neural_network(x=x, y=y):
...
#    tf.reset_default_graph()
    ...
    result = list(zip(jobs, list(result[0])))
#    print(result)
    print('The best match to the description given is ___' + str(sorted(result, key=lambda x: x[1], reverse=True)[0][0]) + '___')
